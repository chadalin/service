...
<!-- Для тестирования -->
<div class="mt-3">
    <h6>Тестовые ссылки:</h6>
    <div class="d-flex gap-2">
        <form action="/admin/documents-processing/parse/{{ $document->id }}" method="POST" class="d-inline">
            @csrf
            <input type="hidden" name="preview" value="true">
            <button type="submit" class="btn btn-sm btn-warning">Прямой POST запрос</button>
        </form>
        
        <a href="javascript:void(0)" onclick="testAjax()" class="btn btn-sm btn-info">Тест AJAX</a>
    </div>
</div>

<script>
function testAjax() {
    fetch('/admin/documents-processing/parse/{{ $document->id }}', {
        method: 'POST',
        headers: {
...
<!-- Простое модальное окно для прогресса -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="text-center">
                    ...
                    <p id="processingMessage">Обработка...</p>
                </div>
            </div>
        </div>
    </div>
...
function createPreview() {
    ...
    $.ajax({
        url: `/admin/documents-processing/${documentId}/parse`,
        method: 'POST',
        data: { 
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                setTimeout(() => {
                    processingModal.hide();
                    showToast('Ошибка: ' + response.message, 'danger');
                }, 2000);
            }
        },
        error: function(xhr, status, error) {
            console.error('AJAX Error:', xhr.responseText);
            updateProcessingModal('Ошибка сервера: ' + xhr.status, 100);
            
            setTimeout(() => {
                processingModal.hide();
                showToast('Ошибка сервера: ' + xhr.status, 'danger');
            }, 2000);
        }
    });
}
